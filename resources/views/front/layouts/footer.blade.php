    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-7 slog-right" dir="{{ direction() }}">
                ...
                <div class="links">
                    <a href="{{route('front.condition_and_terms')}}">{{trans('messages.terms')}} <i
                            class="fal fa-chevron-{{ arrow('left','right') }}"></i></a>
                    <a href="{{route('front.description')}}">{{ trans('messages.settings.description') }}<i
                            class="fal fa-chevron-{{ arrow('left','right') }}"></i></a>
                    <a href="{{route('front.privacy')}}">{{ trans('messages.settings.privacy') }}<i
                            class="fal fa-chevron-{{ arrow('left','right') }}"></i></a>
                </div>
            </div>
...
{{--                                    <li>--}}
{{--                                        <a href="{{route('front.auction_details',$category->auctions->last()->id)}}">{{ substr($category->auctions->last()->$name,0,25) }}</a>--}}
{{--                                    </li>--}}
{{--                                @endforeach--}}
{{--                            </ul>--}}
                        </div>
                    ...
                    <div class="col-12">
                        <div class="ul-parent">
                            ...
                            <div class="social">
                                <a href="{{App\Models\Setting::where('key','facebook_url')->first()->value}}"
                                   target="_blank"><i class="fab fa-snapchat-square"></i></a>
                                <a href="{{ App\Models\Setting:: where('key','instagram_url')->first()->value}}"
                                   target="_blank"><i class="fab fa-instagram"></i></a>
                                <a href="{{ App\Models\Setting:: where('key','twitter_url')->first()->value}}"
