...
//        $code = (int)str_replace(" ", "", $response);
        Log::info($response);
        dd($response);
//        Log::info(self::get_unifonic_message_by_code($code));
        return ['code' => $code, 'message' => self::get_unifonic_message_by_code($code)];

    }



    public static function unifonic_parmeters( $appSid, $senderID, $recipient,$body,
                                               $responseType = null, $correlationID = null, $baseEncode = null, $statusCallback = null, $async = false)
    {
        return [
            'AppSid'     => $appSid,
            'SenderID'   => $senderID,
    ...
    public static function get_unifonic_message_by_code($code)
    {
        if ($code)
        {
